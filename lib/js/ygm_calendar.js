...
function initCalenar(option){
    ...
    var toolbar = "";
    var table = "";
    var s = dateList[0].getDay();
    var length = dateList.length-1;
    var end = 6 - (dateList[length].getDay());
    var endIdx  = s + end + length;
    toolbar += "<div class='ygm-c-toolbar'><h2 class=''>"+monthTxt+"월</h2></div>";
    table += "<table class='ygm-c-content'>";
    table += "<thead class='ygm-c-head'>";
    table += "<tr><td>일</td><td>월</td><td>화</td><td>수</td><td>목</td><td>금</td><td>토</td></tr>";
    table += "</thead>";
    table += "<tbody class='ygm-c-body'>";
    for(var i = 0; i < endIdx; i++) {
        
        if (i == 0 || i % 7 == 0) {
            table += "<tr>";
            table += "<td>"+dateList[i].getDate()+"</td>"
        ...
        // if(!dateList[-6]) tr.append("<td></td>")
    }
    table += "</tbody>";
    table += "</table>";
    
    $("#ygm-calendar").append("<div class='ygm-calendar-wrapper'></div>");
