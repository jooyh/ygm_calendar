...
class ygmCalendar{
    ...
    setCalendarElForMonth(date,dateList,el){
        ...
        for(var i=0; i<endIdx; i++){
            ...
            if(dateList[realIdx]){
                ...
                if(dateList[realIdx].format("yyyy-MM-dd") == this.today.format("yyyy-MM-dd")){
                    cel.classList.add("today-color");
                };
                cel.appendChild(txt);
            }
            tr.appendChild(cel);
    ...
    /**
     * @name addEventToSelDate
     * @param {Object} event
     * @description 일정추가 (선택된 날짜)
     */
    addEventToSelDate = function(event){
        ...
        for(var i=0; i<cels.length; i++){
            ...
            evntWrap.setAttribute("data-ygm-evnt",JSON.stringify(event));
    ...
    setEvent = function(event){
        ...
        for(var i=0; i<cels.length; i++){
            if(evnt.date == cels[i].getAttribute("date")){
                ...
                this.addEvent(event.title,event.desc,event.time);
            }
        }
    }

    /**
     * @name getAllEventByDate
     * @description 일자별 모든 일정 조회     
     */
